...
void
RMSDAndReplace(double ** ref_xlist,
               double** mov_xlist,
               int n_list,
               double* rmsd,
               float* result)
{
  ...
  calculate_rotation_rmsd(ref_xlist, mov_xlist, n_list, U, rmsd);
  for (int i=0; i<n_list; i++)
    {
      double a = U[0][0]*mov_xlist[i][0] + U[0][1]*mov_xlist[i][1]
        + U[0][2]*mov_xlist[i][2];
      double b = U[1][0]*mov_xlist[i][0] + U[1][1]*mov_xlist[i][1]
        + U[1][2]*mov_xlist[i][2];
      double c = U[2][0]*mov_xlist[i][0] + U[2][1]*mov_xlist[i][1]
        + U[2][2]*mov_xlist[i][2];
      result[3*i]   = (float)a;
      result[3*i+1] = (float)b;
      result[3*i+2] = (float)c;
    }
}
